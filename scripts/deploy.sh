...
# Configuration
DEPLOY_DIR="/opt/maiqr-nostr-auth"
PM2_NAME="nostr-auth-middleware"
BACKUP_DIR="/opt/backups/nostr-auth"
...
echo -e "${GREEN}Starting Nostr Auth Middleware deployment...${NC}"
...
if [ -d $DEPLOY_DIR ]; then
    ...
    if [ -f $DEPLOY_DIR/.env ]; then
        # Verify this is indeed a Nostr Auth env file
        if grep -q "NOSTR_" $DEPLOY_DIR/.env || grep -q "SUPABASE_" $DEPLOY_DIR/.env; then
...
if [ -f $BACKUP_ENV ] && [ -f $BACKUP_TIMESTAMP ]; then
    ...
    if [ $BACKUP_AGE -lt 3600 ]; then
        ...
        if grep -q "NOSTR_" $BACKUP_ENV || grep -q "SUPABASE_" $BACKUP_ENV; then
            cp $BACKUP_ENV .env
        else
